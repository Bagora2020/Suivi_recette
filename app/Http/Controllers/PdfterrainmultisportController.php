...
class PdfterrainmultisportController extends Controller
{
    public function index($id)
    {
        ...
        $total_lettre = $this->numberToWord($invoice2->montant);
        $pdf = Pdf::loadView('ordrederecettepdf.pdfterrainmultisport', compact('invoice2', 'total_lettre'));
    ...
    private function numberToWord($number)
{
    ...
    if ($number < 10) {
        ...
        if ($thousands_digit == 1) {
            ...
            $total_lettre = $this->numberToWord($thousands_digit) . ' Mille';
        }
        if ($remainder > 0) {
            if ($remainder >= 100) {
                $total_lettre .= ' ' . $this->numberToWord($remainder);
        ...
        if ($remainder > 0) {
            if ($remainder == 1000) {
                $total_lettre .= ($millions_digit > 0 ? ' ' : '') . 'Mille';
