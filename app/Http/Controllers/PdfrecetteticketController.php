...
class PdfrecetteticketController extends Controller
{
    public function index($id)
    {

        $invoice = recetteticket::find($id);
        $total_lettre = $this->numberToWord($invoice->montant);

        $recetteticket = recetteticket::find($id);
        $typeticket = Typeticket::find($id);

        $pdf = Pdf::loadView('ordrederecettepdf.pdfrecetteticket', compact('invoice', 'recetteticket', 'typeticket', 'total_lettre'));
